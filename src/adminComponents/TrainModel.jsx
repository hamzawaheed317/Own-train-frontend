...
const TrainModel = () => {
  ...
  useEffect(() => {
    const fetchStoredFiles = async () => {
      try {
        const response = await fetch(`https://own-train-backend.vercel.app/admin/files`, {
          method: "GET",
          credentials: "include",
          headers: {
  ...
  const addTrainingMaterial = () => {
    if (uploadedFiles.length > 0 || textInput.trim()) {
      ...
      setIsTraining(true);
      // Send to backend
      fetch(`https://own-train-backend.vercel.app/admin/upload-multiple`, {
        method: "POST",
  ...
  const untrainMaterial = async (id) => {
    ...
    const deleteResponse = await fetch(`https://own-train-backend.vercel.app/admin/files`, {
      method: "DELETE",
      headers: {
